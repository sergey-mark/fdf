...
int			mouse_motion_function(int x, int y, t_wind *w)
{
	...
	if (w->p.paint == 0)
	{
		...
		{
			w->p.x_spacing = w->p.m.mem_spacing_x + (int)(((float)(x -
			w->p.m.memm_x) / (float)w->img.width) * (float)spacing_sens);
			w->p.zaccentuation = w->p.m.mem_zaccentuation - (int)(((float)(y -
			w->p.m.memm_y) / (float)w->img.height) * (float)spacing_sens);
		}
